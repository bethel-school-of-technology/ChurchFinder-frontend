...
//     handleSubmit = (e) => {
//         e.preventDefault()
//         const { name, description, mailing_one, maling_two, city, state, postalcode, denomination, web_url, latitude, longitude } = this.state;

//         axios.post("http://localhost:5000/churches/addchurch", { name, description, mailing_one, maling_two, city, state, postalcode, denomination, web_url, latitude, longitude })
        ...
        axios.post("http://localhost:5000/churches/addchurch", this.state)
            .then(response => {
                ...
                if (response.data.status == 400) {
                    window.alert("church already exists")
                    this.props.history.push("/addchurch")
                } else {
                this.setState({
                    result: response
                });
